    ...
    <va-card-content>
      ...
      >
        ...
        <template #cell(is_parce)="{ rowData }">
          <va-button
            v-if="rowData.is_parce == 1"
            :color="getStatusColor(rowData.is_parce)"
            @click="updateStatus(rowData.id)"
            >Запущен</va-button
          >
          <va-button v-else :color="getStatusColor(rowData.is_parce)" @click="updateStatus(rowData.id)"
    ...
    methods: {
      ...
      deleteSource(id) {
        ...
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/groups/delete-source', { id_source: id }).then((response) => {
            if (response.status) {
