<template>
  <va-card>
    <va-card-content>
      ...
      <div class="row">
        <div class="col">
          <va-button @click="deleteQueue"> Удалить из очереди </va-button>
        </div>
        <div class="col">
      ...
      >
        ...
        <template #cell(text)="{ rowData }">
          <div v-if="rowData.text == ''">
            <a class="link" target="_blank" :href="'https://vk.com/' + rowData.link">Посмотреть</a>
          </div>
          <div v-else>
            <a class="link" target="_blank" :href="'https://vk.com/' + rowData.link">{{ rowData.text }}</a>
          </div>
        </template>
        <template #bodyAppend>
          <tr>
    ...
    methods: {
      ...
      fetchRows() {
        ...
        axios
          .request({
            method: 'post',
            url: '/api/queue/telegramm-video-sexy',
      ...
      publish: function (event, rows) {
        ...
        console.log(self.selectedItemsEmitted)
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/post/vk-anime-publish', { selRows: self.selectedItemsEmitted }).then((response) => {
            if (response.status) {
              console.log('Вызвали алерт')
      ...
      deleteQueue: function (event, rows) {
        ...
        console.log(self.selectedItemsEmitted)
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/queue/unset', { selRows: self.selectedItemsEmitted }).then((response) => {
            if (response.status) {
              console.log('Вызвали алерт')
