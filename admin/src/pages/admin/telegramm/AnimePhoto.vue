  ...
  <va-card>
    <va-card-content>
      ...
      <div class="row">
        ...
        <div class="col">
          <va-button @click="addQueue"> Добавить в очередь </va-button>
        </div>
        <div class="col">
          <va-button color="danger" @click="hidden"> Скрыть </va-button>
        </div>
        <div class="col">
          <va-button color="success" @click="createAdvertPost"> Создать пост с кнопкой </va-button>
        </div>
        <div class="col">
          <va-button color="danger" @click="deletePost"> Удалить </va-button>
        </div>
      </div>
      <va-data-table
        :items="items"
      ...
      >
        ...
        <template #cell(text)="{ rowData }">
          <div v-if="rowData.text == ''"><a class="link" target="_blank" :href="rowData.link">Посмотреть</a></div>
          <div v-else>
            <a class="link" target="_blank" :href="rowData.link">{{ rowData.text }}</a>
          </div>
        </template>
        <template #bodyAppend>
          <tr>
    ...
    methods: {
      ...
      fetchRows() {
        ...
        axios
          .request({
            method: 'post',
            url: '/api/telegramm/anime-photo-all',
      ...
      hidden: function (event, rows) {
        ...
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/post/post-hidden', { selRows: self.selectedItemsEmitted }).then((response) => {
            if (response.status) {
      ...
      deletePost: function (event, rows) {
        ...
        console.log(self.selectedItemsEmitted)
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/post/delete', { selRows: self.selectedItemsEmitted }).then((response) => {
            if (response.status) {
              console.log('Вызвали алерт')
      ...
      publish: function (event, rows) {
        ...
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios
            .post('/api/telegramm/anime-photo-telegram-publish', { selRows: self.selectedItemsEmitted })
            .then((response) => {
              if (response.status) {
                console.log('Вызвали алерт')
      ...
      addQueue: function (event, rows) {
        ...
        console.log(self.selectedItemsEmitted)
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios.post('/api/queue/set', { selRows: self.selectedItemsEmitted }).then((response) => {
            if (response.status) {
              console.log('Вызвали алерт')
      ...
      createAdvertPost: function (event, rows) {
        let self = this
        this.loading = true
        console.log(self.selectedItemsEmitted)
        axios.get('/sanctum/csrf-cookie').then((response) => {
          axios
            .post('/api/tumblr/sexy-advert-photo-tumblr-publish', { selRows: self.selectedItemsEmitted })
            .then((response) => {
              if (response.status) {
                console.log('Вызвали алерт')
                this.$vaToast.init({ message: 'Запись добавлена в телеграмм', color: 'success' })
